...
import com.sun.dragrv.DragRecyclerView.OnItemTouchListener;
import com.sun.dragrv.DragRecyclerView.ViewHolder;
...
/**
 * This is a utility class to add swipe to dismiss and drag & drop support to DragRecyclerView.
 * <p>
 * It works with a DragRecyclerView and a Callback class, which configures what type of interactions
 * are enabled and also receives events when user performs these actions.
 * <p>
 * Depending on which functionality you support, you should override
 * {@link Callback#onMove(DragRecyclerView, ViewHolder, ViewHolder)} and / or
 ...
 * overriding {@link Callback#onChildDraw(Canvas, DragRecyclerView, ViewHolder, float, float, int,
 * boolean)}
 * or {@link Callback#onChildDrawOver(Canvas, DragRecyclerView, ViewHolder, float, float, int,
 * boolean)}.
 * <p/>
 ...
 */
public class ItemTouchHelper extends DragRecyclerView.ItemDecoration
        implements DragRecyclerView.OnChildAttachStateChangeListener {
    ...
    /**
     * Horizontal start direction. Resolved to LEFT or RIGHT depending on DragRecyclerView's layout
     * direction. Used for swipe & drag control.
     */
    public static final int START = LEFT << 2;

    /**
     * Horizontal end direction. Resolved to LEFT or RIGHT depending on DragRecyclerView's layout
     * direction. Used for swipe & drag control.
     */
    ...
    /**
     * Views, whose state should be cleared after they are detached from DragRecyclerView.
     ...
     * {@link Callback#getAbsoluteMovementFlags(DragRecyclerView, ViewHolder)} for the current
     * action state.
     */
    int mSelectedFlags;
     ...
     */
    private DragRecyclerView.ChildDrawingOrderCallback mChildDrawingOrderCallback = null;
    ...
    private final OnItemTouchListener mOnItemTouchListener
        ...
        @Override
        public boolean onInterceptTouchEvent(DragRecyclerView recyclerView, final MotionEvent event) {
            mGestureDetector.onTouchEvent(event);
            if (DEBUG) {
                sLog.d(TAG, "intercept: x:" + event.getX() + ",y:" + event.getY() + ", " + event);
            }
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                recyclerView.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        sLog.e("RunTime", "-------ACTION_DOWN---------->" );
                        if(Math.abs(lastX-event.getRawX())<=10&&Math.abs(lastY-event.getRawY())<=10){
                            sLog.e("RunTime", "-------ACTION_DOWN---------->"+mSelected );
                            mCallback.onStartDrag(mSelected,event.getRawX(),event.getRawY());
                        }
                    }
                },500);
                //add end
                ...
                if (DEBUG) {
                    sLog.d(TAG, "pointer index " + index);
                }
                if (index >= 0) {
        ...
        @Override
        public void onTouchEvent(DragRecyclerView recyclerView, final MotionEvent event) {
            mGestureDetector.onTouchEvent(event);
            if (DEBUG) {
                sLog.d(TAG,
            ...
            final ViewHolder viewHolder = mSelected;
            if (viewHolder == null) {
                return;
            }
            ...
            switch (action) {
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if (activePointerIndex >= 0) {
                        ...
                        mCallback.onDrag(viewHolder,event.getRawX(),event.getRawY());
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    mActivePointerId = ACTIVE_POINTER_ID_NONE;
                    //add by sunhy
                    mCallback.onDragOver();
                    if(viewHolder.itemView.getVisibility()==View.INVISIBLE){
    ...
    private float lastX;
    private float lastY;
    boolean isLongPressed(float thisX, float thisY,  long lastDownTime, long thisEventTime) {
        float offsetX = Math.abs(thisX - lastX);
        float offsetY = Math.abs(thisY - lastY);
        long intervalTime = thisEventTime - lastDownTime;
        lastX = thisX;
        lastY = thisY;
        sLog.e("RunTime", "x---> "+offsetX+" --y--> "+offsetY+" --time--> "+intervalTime );
        if (offsetX <= 30 && offsetY <= 30 && intervalTime>=500) {
            return true;
        }
        return false;
    }
     ...
     * <p>
     * You can attach ItemTouchHelper to a DragRecyclerView via
     * {@link #attachToRecyclerView(DragRecyclerView)}. Upon attaching, it will add an item decoration,
     * an onItemTouchListener and a Child attach / detach listener to the DragRecyclerView.
     *
    ...
    /**
     * Attaches the ItemTouchHelper to the provided DragRecyclerView. If TouchHelper is already
     * attached
     * to a DragRecyclerView, it will first detach from the previous one.
     *
     * @param recyclerView The DragRecyclerView instance to which you want to add this helper.
     */
    public void attachToRecyclerView(DragRecyclerView recyclerView) {
    ...
    @Override
    public void onDrawOver(Canvas c, DragRecyclerView parent, DragRecyclerView.State state) {
        float dx = 0, dy = 0;
        if (mSelected != null) {
    ...
    @Override
    public void onDraw(Canvas c, DragRecyclerView parent, DragRecyclerView.State state) {
        ...
        mRecyclerView.post(new Runnable() {
            ...
            public void run() {
                if (mRecyclerView != null && mRecyclerView.isAttachedToWindow() &&
                        !anim.mOverridden &&
                        anim.mViewHolder.getAdapterPosition() != DragRecyclerView.NO_POSITION) {
                    final DragRecyclerView.ItemAnimator animator = mRecyclerView.getItemAnimator();
    ...
    private boolean scrollIfNecessary() {
        ...
        DragRecyclerView.LayoutManager lm = mRecyclerView.getLayoutManager();
        if (mTmpRect == null) {
            mTmpRect = new Rect();
        }
        int scrollX = 0;
    ...
    private List<ViewHolder> findSwapTargets(ViewHolder viewHolder) {
        ...
        final DragRecyclerView.LayoutManager lm = mRecyclerView.getLayoutManager();
    ...
    @Override
    public void getItemOffsets(Rect outRect, View view, DragRecyclerView parent,
            DragRecyclerView.State state) {
        outRect.setEmpty();
    }
    ...
    private boolean checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        if (mRecyclerView.getScrollState() == DragRecyclerView.SCROLL_STATE_DRAGGING) {
            return false;
        }
     ...
     * <ul>
     * <li>The provided ViewHolder must be a child of the DragRecyclerView to which this
     * ItemTouchHelper
     * is attached.</li>
     ...
     * through DragRecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
    ...
    public void startDrag(ViewHolder viewHolder) {
        if (!mCallback.hasDragFlag(mRecyclerView, viewHolder)) {
            sLog.e(TAG, "Start drag has been called but swiping is not enabled");
            return;
        }
        if (viewHolder.itemView.getParent() != mRecyclerView) {
            sLog.e(TAG, "Start drag has been called with a view holder which is not a child of "
                    + "the DragRecyclerView which is controlled by this ItemTouchHelper.");
            return;
        }
        obtainVelocityTracker();
        mDx = mDy = 0f;
    ...
    public void startSwipe(ViewHolder viewHolder) {
        if (!mCallback.hasSwipeFlag(mRecyclerView, viewHolder)) {
            sLog.e(TAG, "Start swipe has been called but dragging is not enabled");
            return;
        }
        if (viewHolder.itemView.getParent() != mRecyclerView) {
            sLog.e(TAG, "Start swipe has been called with a view holder which is not a child of "
                    + "the DragRecyclerView controlled by this ItemTouchHelper.");
            return;
        }
        obtainVelocityTracker();
        mDx = mDy = 0f;
    ...
    private void addChildDrawingOrderCallback() {
        ...
        if (mChildDrawingOrderCallback == null) {
            mChildDrawingOrderCallback = new DragRecyclerView.ChildDrawingOrderCallback() {
                @Override
     ...
     * {@link #getMovementFlags(DragRecyclerView, ViewHolder)} and return appropriate set
     ...
     * {@link Callback#onMove(DragRecyclerView, ViewHolder, ViewHolder)
     ...
     * in your adapter and also call {@link DragRecyclerView.Adapter#notifyItemMoved(int, int)}.
     * To control where a View can be dropped, you can override
     * {@link #canDropOver(DragRecyclerView, ViewHolder, ViewHolder)}. When a
    ...
    public abstract static class Callback {
        ...
        //add by sunhy
        public abstract void onStartDrag(DragRecyclerView.ViewHolder view,float x,float y);

        public abstract void onDragOver();

        public abstract void onDrag(DragRecyclerView.ViewHolder view,float x,float y);
        //add end
        ...
        static {
            if (Build.VERSION.SDK_INT >= 21) {
                sUICallback = new com.sun.dragrv.helper.ItemTouchUIUtilImpl.Lollipop();
            } else if (Build.VERSION.SDK_INT >= 11) {
                sUICallback = new com.sun.dragrv.helper.ItemTouchUIUtilImpl.Honeycomb();
            } else {
                sUICallback = new com.sun.dragrv.helper.ItemTouchUIUtilImpl.Gingerbread();
            }
        }
         ...
         * <pre>
         *     public void clearView(DragRecyclerView recyclerView, DragRecyclerView.ViewHolder viewHolder){
         *         getDefaultUIUtil().clearView(((ItemTouchViewHolder) viewHolder).textView);
         *     }
         *     public void onSelectedChanged(DragRecyclerView.ViewHolder viewHolder, int actionState) {
         *         if (viewHolder != null){
         *             getDefaultUIUtil().onSelected(((ItemTouchViewHolder) viewHolder).textView);
         *         }
         *     }
         *     public void onChildDraw(Canvas c, DragRecyclerView recyclerView,
         *             DragRecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState,
         ...
         *         return true;
         *     }
         *     public void onChildDrawOver(Canvas c, DragRecyclerView recyclerView,
         *             DragRecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState,
         *             boolean isCurrentlyActive) {
         ...
         * swiping.
         *
         * @param recyclerView The DragRecyclerView to which ItemTouchHelper is attached.
         * @param viewHolder   The ViewHolder for which the movement information is necessary.
         * @return flags specifying which movements are allowed on this ViewHolder.
         * @see #makeMovementFlags(int, int)
         * @see #makeFlag(int, int)
         */
        public abstract int getMovementFlags(DragRecyclerView recyclerView,
         ...
         *
         * @param flags           The flag value that include any number of movement flags.
         * @param layoutDirection The layout direction of the DragRecyclerView.
        ...
        final int getAbsoluteMovementFlags(DragRecyclerView recyclerView,
                ViewHolder viewHolder) {
            final int flags = getMovementFlags(recyclerView, viewHolder);
            return convertToAbsoluteDirection(flags, ViewCompat.getLayoutDirection(recyclerView));
        }

        private boolean hasDragFlag(DragRecyclerView recyclerView, ViewHolder viewHolder) {
            final int flags = getAbsoluteMovementFlags(recyclerView, viewHolder);
            return (flags & ACTION_MODE_DRAG_MASK) != 0;
        }

        private boolean hasSwipeFlag(DragRecyclerView recyclerView,
                ViewHolder viewHolder) {
            final int flags = getAbsoluteMovementFlags(recyclerView, viewHolder);
         ...
         *
         * @param recyclerView The DragRecyclerView to which ItemTouchHelper is attached to.
         ...
         * otherwise.
         */
        public boolean canDropOver(DragRecyclerView recyclerView, ViewHolder current,
                ViewHolder target) {
            return true;
        }
         ...
         * {@code target}.
         * @see #onMoved(DragRecyclerView, ViewHolder, int, ViewHolder, int, int, int)
         */
        public abstract boolean onMove(DragRecyclerView recyclerView,
         ...
         * The fraction is calculated with respect to DragRecyclerView's bounds.
         * <p>
         * Default value is .5f, which means, to swipe a View, user must move the View at least
         * half of DragRecyclerView's width or height, depending on the swipe direction.
         *
         ...
         *                    {@link DragRecyclerView.ItemDecoration}s.
         ...
         * {@link #getMovementFlags(DragRecyclerView, ViewHolder)} method, this method
         ...
         * {@link #clearView(DragRecyclerView, ViewHolder)}.
         *
         * @param viewHolder The ViewHolder which has been swiped by the user.
         * @param direction  The direction to which the ViewHolder is swiped. It is one of
         *                   {@link #UP}, {@link #DOWN},
         *                   {@link #LEFT} or {@link #RIGHT}. If your
         *                   {@link #getMovementFlags(DragRecyclerView, ViewHolder)}
         *                   method
         ...
         *
         * @see #clearView(DragRecyclerView, ViewHolder)
         */
        ...
        private int getMaxDragScroll(DragRecyclerView recyclerView) {
        ...
        /**
         * Called when {@link #onMove(DragRecyclerView, ViewHolder, ViewHolder)} returns true.
         * <p>
         * ItemTouchHelper does not create an extra Bitmap or View while dragging, instead, it
         * modifies the existing View. Because of this reason, it is important that the View is
         * still part of the layout after it is moved. This may not work as intended when swapped
         * Views are close to DragRecyclerView bounds or there are gaps between them (e.g. other Views
         ...
         *
         * Default implementation calls {@link DragRecyclerView#scrollToPosition(int)} if the View's
         ...
         *
         * @param recyclerView The DragRecyclerView controlled by the ItemTouchHelper.
         ...
         *                     {@link DragRecyclerView.ItemDecoration}s.
         */
        public void onMoved(final DragRecyclerView recyclerView,
                final ViewHolder viewHolder, int fromPos, final ViewHolder target, int toPos, int x,
                int y) {
            final DragRecyclerView.LayoutManager layoutManager = recyclerView.getLayoutManager();
        ...
        private void onDraw(Canvas c, DragRecyclerView parent, ViewHolder selected,
        ...
        private void onDrawOver(Canvas c, DragRecyclerView parent, ViewHolder selected,
                List<ItemTouchHelper.RecoverAnimation> recoverAnimationList,
                int actionState, float dX, float dY) {
            final int recoverAnimSize = recoverAnimationList.size();
            for (int i = 0; i < recoverAnimSize; i++) {
         ...
         * {@link #onChildDraw(Canvas, DragRecyclerView, ViewHolder, float, float, int,
         * boolean)} or
         * {@link #onChildDrawOver(Canvas, DragRecyclerView, ViewHolder, float, float, int, boolean)}.
         *
         * @param recyclerView The DragRecyclerView which is controlled by the ItemTouchHelper.
         * @param viewHolder   The View that was interacted by the user.
         */
        public void clearView(DragRecyclerView recyclerView, ViewHolder viewHolder) {
            sUICallback.clearView(viewHolder.itemView);
        }

        /**
         * Called by ItemTouchHelper on DragRecyclerView's onDraw callback.
         * <p>
         ...
         *
         * @param c                 The canvas which DragRecyclerView is drawing its children
         * @param recyclerView      The DragRecyclerView to which ItemTouchHelper is attached to
         ...
         * @see #onChildDrawOver(Canvas, DragRecyclerView, ViewHolder, float, float, int,
         * boolean)
         */
        public void onChildDraw(Canvas c, DragRecyclerView recyclerView,
                ViewHolder viewHolder,
         ...
         * {@link DragRecyclerView.ItemAnimator#getMoveDuration()}, otherwise, it returns
         * {@link DragRecyclerView.ItemAnimator#getRemoveDuration()}. If DragRecyclerView does not have
         * any {@link DragRecyclerView.ItemAnimator} attached, this method returns
         * {@code DEFAULT_DRAG_ANIMATION_DURATION} or {@code DEFAULT_SWIPE_ANIMATION_DURATION}
         * depending on the animation type.
         *
         * @param recyclerView  The DragRecyclerView to which the ItemTouchHelper is attached to.
         ...
         */
        public long getAnimationDuration(DragRecyclerView recyclerView, int animationType,
                float animateDx, float animateDy) {
            final DragRecyclerView.ItemAnimator itemAnimator = recyclerView.getItemAnimator();
         ...
         * <p>
         * You can override this method to decide how much DragRecyclerView should scroll in response
         ...
         * bounds, the faster the DragRecyclerView will scroll.
         *
         * @param recyclerView        The DragRecyclerView instance to which ItemTouchHelper is attached
         *                            to.
         ...
         * @param totalSize           The total size of DragRecyclerView in the scroll direction.
         * @param msSinceStartScroll  The time passed since View is kept out of bounds.
         *
         * @return The amount that DragRecyclerView should scroll. Keep in mind that this value will
         * be passed to {@link DragRecyclerView#scrollBy(int, int)} method.
         */
        public int interpolateOutOfBoundsScroll(DragRecyclerView recyclerView,
    ...
    public abstract static class SimpleCallback extends Callback {
         ...
         * {@link #getSwipeDirs(DragRecyclerView, ViewHolder)}
         * and / or {@link #getDragDirs(DragRecyclerView, ViewHolder)}.
         *
         ...
         *
         * @param recyclerView The DragRecyclerView to which the ItemTouchHelper is attached to.
         * @param viewHolder   The DragRecyclerView for which the swipe drection is queried.
         * @return A binary OR of direction flags.
         */
        public int getSwipeDirs(DragRecyclerView recyclerView, ViewHolder viewHolder) {
         ...
         * @return A binary OR of direction flags.
         */
        public int getDragDirs(DragRecyclerView recyclerView, ViewHolder viewHolder) {
            return mDefaultDragDirs;
        }

        @Override
        public int getMovementFlags(DragRecyclerView recyclerView, ViewHolder viewHolder) {
