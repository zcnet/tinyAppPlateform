...
class AdapterHelper implements OpReorderer.Callback {
    ...
    private void dispatchAndUpdateViewHolders(UpdateOp op) {
        ...
        if (DEBUG) {
            sLog.d(TAG, "dispatch (pre)" + op);
            sLog.d(TAG, "postponed state before:");
            for (UpdateOp updateOp : mPostponedList) {
                sLog.d(TAG, updateOp.toString());
            }
            sLog.d(TAG, "----");
        }
        ...
        if (DEBUG) {
            sLog.d(TAG, "pos:" + op.positionStart + ",updatedPos:" + tmpStart);
        }
        int tmpCnt = 1;
        ...
        for (int p = 1; p < op.itemCount; p++) {
            ...
            if (DEBUG) {
                sLog.d(TAG, "pos:" + pos + ",updatedPos:" + updatedPos);
            }
            ...
            if (continuous) {
                ...
                if (DEBUG) {
                    sLog.d(TAG, "need to dispatch separately " + tmp);
                }
        ...
        if (tmpCnt > 0) {
            ...
            if (DEBUG) {
                sLog.d(TAG, "dispatching:" + tmp);
            }
            dispatchFirstPassAndUpdateViewHolders(tmp, offsetPositionForPartial);
            recycleUpdateOp(tmp);
        }
        if (DEBUG) {
            sLog.d(TAG, "post dispatch");
            sLog.d(TAG, "postponed state after:");
            for (UpdateOp updateOp : mPostponedList) {
                sLog.d(TAG, updateOp.toString());
            }
            sLog.d(TAG, "----");
    ...
    private int updatePositionWithPostponed(int pos, int cmd) {
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (DEBUG) {
                sLog.d(TAG, "dispath (step" + i + ")");
                sLog.d(TAG, "postponed state:" + i + ", pos:" + pos);
                for (UpdateOp updateOp : mPostponedList) {
                    sLog.d(TAG, updateOp.toString());
                }
                sLog.d(TAG, "----");
