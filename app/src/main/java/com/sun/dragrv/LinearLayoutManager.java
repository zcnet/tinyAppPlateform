...
/**
 * A {@link DragRecyclerView.LayoutManager} implementation which provides
 * similar functionality to {@link android.widget.ListView}.
 */
public class LinearLayoutManager extends DragRecyclerView.LayoutManager implements
...
//     * {@link #onLayoutChildren(DragRecyclerView.Recycler, DragRecyclerView.State)} is run.
     */
     ...
     */
    int mPendingScrollPosition = DragRecyclerView.NO_POSITION;
    ...
    * */
    ...
    public LinearLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                               int defStyleRes) {
        Properties properties = DragRecyclerView.LayoutManager.getProperties(context, attrs, defStyleAttr, defStyleRes);
     ...
     */
    @Override
    public DragRecyclerView.LayoutParams generateDefaultLayoutParams() {
        return new DragRecyclerView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
     ...
     * RecyclerView.
     * <p>
     * If you are using a {@link DragRecyclerView.RecycledViewPool}, it might be a good idea to set
    ...
    @Override
    public void onDetachedFromWindow(DragRecyclerView view, DragRecyclerView.Recycler recycler) {
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof SavedState) {
            ...
            requestLayout();
            if (DEBUG) {
                sLog.d(TAG, "loaded saved state");
            }
        } else if (DEBUG) {
            sLog.d(TAG, "invalid saved state class");
        }
    }
     ...
     * When set to true, If {@link DragRecyclerView} is LTR, than it will
     * layout from RTL, if {@link DragRecyclerView}} is RTL, it will layout
     * from LTR.
     *
     ...
     */
    protected int getExtraLayoutSpace(DragRecyclerView.State state) {
    ...
    @Override
    public void smoothScrollToPosition(DragRecyclerView recyclerView, DragRecyclerView.State state,
            int position) {
     ...
     */
    @Override
    public void onLayoutChildren(DragRecyclerView.Recycler recycler, DragRecyclerView.State state) {
        // layout algorithm:
        ...
        // create layout state
        if (DEBUG) {
            sLog.d(TAG, "is pre layout:" + state.isPreLayout());
        }
        if (mPendingSavedState != null || mPendingScrollPosition != DragRecyclerView.NO_POSITION) {
        ...
        if (DEBUG) {
            sLog.d(TAG, "Anchor info:" + mAnchorInfo);
        }
        ...
        if (state.isPreLayout() && mPendingScrollPosition != DragRecyclerView.NO_POSITION &&
        ...
        if (!state.isPreLayout()) {
            mPendingScrollPosition = DragRecyclerView.NO_POSITION;
            mPendingScrollPositionOffset = INVALID_OFFSET;
     ...
     */
    void onAnchorReady(DragRecyclerView.Recycler recycler, DragRecyclerView.State state,
     ...
     */
    private void layoutForPredictiveAnimations(DragRecyclerView.Recycler recycler,
                                               DragRecyclerView.State state, int startOffset,  int endOffset) {
        ...
        final List<DragRecyclerView.ViewHolder> scrapList = recycler.getScrapList();
        final int scrapSize = scrapList.size();
        final int firstChildPos = getPosition(getChildAt(0));
        for (int i = 0; i < scrapSize; i++) {
            DragRecyclerView.ViewHolder scrap = scrapList.get(i);
            if (scrap.isRemoved()) {
                continue;
            }
        ...
        if (DEBUG) {
            sLog.d(TAG, "for unused scrap, decided to add " + scrapExtraStart
    ...
    private void updateAnchorInfoForLayout(DragRecyclerView.Recycler recycler, DragRecyclerView.State state,
                                           AnchorInfo anchorInfo) {
        if (updateAnchorFromPendingData(state, anchorInfo)) {
            if (DEBUG) {
                sLog.d(TAG, "updated anchor info from pending information");
            }
            return;
        }
        ...
        if (updateAnchorFromChildren(recycler, state, anchorInfo)) {
            if (DEBUG) {
                sLog.d(TAG, "updated anchor info from existing children");
            }
            return;
        }
        if (DEBUG) {
            sLog.d(TAG, "deciding anchor info for fresh state");
        }
     ...
     */
    private boolean updateAnchorFromChildren(DragRecyclerView.Recycler recycler,
                                             DragRecyclerView.State state, AnchorInfo anchorInfo) {
        if (getChildCount() == 0) {
            return false;
        }
     ...
     */
    private boolean updateAnchorFromPendingData(DragRecyclerView.State state, AnchorInfo anchorInfo) {
        if (state.isPreLayout() || mPendingScrollPosition == DragRecyclerView.NO_POSITION) {
            return false;
        }
        ...
        if (mPendingScrollPosition < 0 || mPendingScrollPosition >= state.getItemCount()) {
            ...
            mPendingScrollPositionOffset = INVALID_OFFSET;
            if (DEBUG) {
                sLog.e(TAG, "ignoring invalid scroll position " + mPendingScrollPosition);
            }
            return false;
        }
     ...
     */
    private int fixLayoutEndGap(int endOffset, DragRecyclerView.Recycler recycler,
                                DragRecyclerView.State state, boolean canOffsetChildren) {
     ...
     * @return The final offset amount for children
     */
    private int fixLayoutStartGap(int startOffset, DragRecyclerView.Recycler recycler,
                                  DragRecyclerView.State state, boolean canOffsetChildren) {
     ...
     */
    @Override
    public int scrollHorizontallyBy(int dx, DragRecyclerView.Recycler recycler,
     ...
     */
    @Override
    public int scrollVerticallyBy(int dy, DragRecyclerView.Recycler recycler,
                                  DragRecyclerView.State state) {
        if (mOrientation == HORIZONTAL) {
            return 0;
        }
    ...
    @Override
    public int computeHorizontalScrollOffset(DragRecyclerView.State state) {
        return computeScrollOffset(state);
    }

    @Override
    public int computeVerticalScrollOffset(DragRecyclerView.State state) {
        return computeScrollOffset(state);
    }

    @Override
    public int computeHorizontalScrollExtent(DragRecyclerView.State state) {
        return computeScrollExtent(state);
    }

    @Override
    public int computeVerticalScrollExtent(DragRecyclerView.State state) {
        return computeScrollExtent(state);
    }

    @Override
    public int computeHorizontalScrollRange(DragRecyclerView.State state) {
        return computeScrollRange(state);
    }

    @Override
    public int computeVerticalScrollRange(DragRecyclerView.State state) {
        return computeScrollRange(state);
    }

    private int computeScrollOffset(DragRecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private int computeScrollExtent(DragRecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private int computeScrollRange(DragRecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
            boolean canUseExistingSpace, DragRecyclerView.State state) {
    ...
    int scrollBy(int dy, DragRecyclerView.Recycler recycler, DragRecyclerView.State state) {
        ...
        if (consumed < 0) {
            if (DEBUG) {
                sLog.d(TAG, "Don't have any more elements to scroll");
            }
            return 0;
        }
        ...
        if (DEBUG) {
            sLog.d(TAG, "scroll req: " + dy + " scrolled: " + scrolled);
        }
     ...
     */
    private void recycleChildren(DragRecyclerView.Recycler recycler, int startIndex, int endIndex) {
        if (startIndex == endIndex) {
            return;
        }
        if (DEBUG) {
            sLog.d(TAG, "Recycling " + Math.abs(startIndex - endIndex) + " items");
        }
     ...
     *
     * @param recycler Recycler instance of {@link DragRecyclerView}
     * @param dt       This can be used to add additional padding to the visible area. This is used
     *                 to detect children that will go out of bounds after scrolling, without
     *                 actually moving them.
     */
    private void recycleViewsFromStart(DragRecyclerView.Recycler recycler, int dt) {
        if (dt < 0) {
            if (DEBUG) {
                sLog.d(TAG, "Called recycle from start with a negative value. This might happen"
     ...
     *                 actually moving them.
     */
    private void recycleViewsFromEnd(DragRecyclerView.Recycler recycler, int dt) {
        final int childCount = getChildCount();
        if (dt < 0) {
            if (DEBUG) {
                sLog.d(TAG, "Called recycle from end with a negative value. This might happen"
                        + " during layout changes but may be sign of a bug");
            }
            return;
        }
     ...
     *
     * @param recycler    Current recycler that is attached to DragRecyclerView
     * @param layoutState Current layout state. Right now, this object does not change but
     *                    we may consider moving it out of this view so passing around as a
     *                    parameter for now, rather than accessing {@link #mLayoutState}
     * @see #recycleViewsFromStart(DragRecyclerView.Recycler, int)
     * @see #recycleViewsFromEnd(DragRecyclerView.Recycler, int)
     * @see LinearLayoutManager.LayoutState#mLayoutDirection
     */
    private void recycleByLayoutState(DragRecyclerView.Recycler recycler, LayoutState layoutState) {
     ...
     */
    int fill(DragRecyclerView.Recycler recycler, LayoutState layoutState,
            DragRecyclerView.State state, boolean stopOnFocusable) {
    ...
    void layoutChunk(DragRecyclerView.Recycler recycler, DragRecyclerView.State state,
        ...
        DragRecyclerView.LayoutParams params = (DragRecyclerView.LayoutParams) view.getLayoutParams();
        ...
        if (DEBUG) {
            sLog.d(TAG, "laid out child at position " + getPosition(view) + ", with l:"
    ...
    private int convertFocusDirectionToLayoutDirection(int focusDirection) {
        switch (focusDirection) {
            ...
            default:
                if (DEBUG) {
                    sLog.d(TAG, "Unknown focus request:" + focusDirection);
                }
     ...
     */
    private View findReferenceChildClosestToEnd(DragRecyclerView.Recycler recycler,
                                                DragRecyclerView.State state) {
     ...
     * @return A View that can be used an an anchor View.
     */
    private View findReferenceChildClosestToStart(DragRecyclerView.Recycler recycler,
                                                  DragRecyclerView.State state) {
        return mShouldReverseLayout ? findLastReferenceChild(recycler, state) :
                findFirstReferenceChild(recycler, state);
    }

    private View findFirstReferenceChild(DragRecyclerView.Recycler recycler, DragRecyclerView.State state) {
        return findReferenceChild(recycler, state, 0, getChildCount(), state.getItemCount());
    }

    private View findLastReferenceChild(DragRecyclerView.Recycler recycler, DragRecyclerView.State state) {
        return findReferenceChild(recycler, state, getChildCount() - 1, -1, state.getItemCount());
    }

    // overridden by GridLayoutManager
    View findReferenceChild(DragRecyclerView.Recycler recycler, DragRecyclerView.State state,
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                if (((DragRecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
     ...
     *
     * @return The adapter position of the first visible item or {@link DragRecyclerView#NO_POSITION} if
    ...
    public int findFirstVisibleItemPosition() {
        ...
        return child == null ? DragRecyclerView.NO_POSITION : getPosition(child);
    }
     ...
     * {@link DragRecyclerView#NO_POSITION} if there aren't any visible items.
     ...
     * not in the layout.
     * <p>
     * If DragRecyclerView has item decorators, they will be considered in calculations as well.
     * <p>
     * LayoutManager may pre-cache some views that are not necessarily visible. Those views
     * are ignored in this method.
     *
     * @return The adapter position of the last visible view or {@link DragRecyclerView#NO_POSITION} if
     * there aren't any visible items.
     * @see #findLastCompletelyVisibleItemPosition()
     * @see #findFirstVisibleItemPosition()
     */
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
                                    DragRecyclerView.Recycler recycler, DragRecyclerView.State state) {
        resolveShouldLayoutReverse();
        if (getChildCount() == 0) {
            return null;
        }
    ...
    private void logChildren() {
        sLog.d(TAG, "internal representation of views on the screen");
        for (int i = 0; i < getChildCount(); i++) {
            View child = getChildAt(i);
            sLog.d(TAG, "item " + getPosition(child) + ", coord:"
                    + mOrientationHelper.getDecoratedStart(child));
        }
        sLog.d(TAG, "==============");
    }
    ...
    void validateChildOrder() {
        sLog.d(TAG, "validating child count " + getChildCount());
        if (getChildCount() < 1) {
            return;
        }
    ...
    static class LayoutState {
        ...
        /**
         * Equal to {@link DragRecyclerView.State#isPreLayout()}. When consuming scrap, if this value
        ...
        /**
         * The most recent {@link #scrollBy(int, DragRecyclerView.Recycler, DragRecyclerView.State)} amount.
         */
         ...
         */
        List<DragRecyclerView.ViewHolder> mScrapList = null;

        /**
         * @return true if there are more items in the data adapter
         */
        boolean hasMore(DragRecyclerView.State state) {
         ...
         */
        View next(DragRecyclerView.Recycler recycler) {
            if (mScrapList != null) {
        ...
        private View nextViewFromScrapList() {
            ...
            for (int i = 0; i < size; i++) {
                ...
                final DragRecyclerView.LayoutParams lp = (DragRecyclerView.LayoutParams) view.getLayoutParams();
                if (lp.isItemRemoved()) {
                    continue;
                }
        ...
        public void assignPositionFromScrapList(View ignore) {
            ...
            if (closest == null) {
                mCurrentPosition = DragRecyclerView.NO_POSITION;
            } else {
                mCurrentPosition = ((DragRecyclerView.LayoutParams) closest.getLayoutParams())
        ...
        void log() {
            sLog.d("sunhy_LinLayoutManager", "avail:" + mAvailable + ", ind:" + mCurrentPosition + ", dir:" +
    ...
    public static class SavedState implements Parcelable {
        ...
        void invalidateAnchor() {
            mAnchorPosition = DragRecyclerView.NO_POSITION;
        }
    ...
    class AnchorInfo {
        ...
        void reset() {
            mPosition = DragRecyclerView.NO_POSITION;
            mCoordinate = INVALID_OFFSET;
            mLayoutFromEnd = false;
        }

        /**
         * assigns anchor coordinate from the DragRecyclerView's padding depending on current
         * layoutFromEnd value
         */
        ...
        private boolean isViewValidAsAnchor(View child, DragRecyclerView.State state) {
            DragRecyclerView.LayoutParams lp = (DragRecyclerView.LayoutParams) child.getLayoutParams();
