...
/**
 * A {@link DragRecyclerView.LayoutManager} implementations that lays out items in a grid.
 * <p>
...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    // re-used variable to acquire decor insets from DragRecyclerView
    final Rect mDecorInsets = new Rect();


    /**
     * Constructor used when layout manager is set in XML by DragRecyclerView attribute
     * "layoutManager". If spanCount is not specified in the XML, it defaults to a
     * single column.
     *
     * @attr ref android.support.v7.DragRecyclerView.R.styleable#DragRecyclerView_spanCount
     */
    ...
    @Override
    public int getRowCountForAccessibility(DragRecyclerView.Recycler recycler,
    ...
    @Override
    public int getColumnCountForAccessibility(DragRecyclerView.Recycler recycler,
            DragRecyclerView.State state) {
    ...
    @Override
    public void onInitializeAccessibilityNodeInfoForItem(DragRecyclerView.Recycler recycler,
                                                         DragRecyclerView.State state, View host, AccessibilityNodeInfoCompat info) {
    ...
    @Override
    public DragRecyclerView.LayoutParams generateLayoutParams(Context c, AttributeSet attrs) {
        return new LayoutParams(c, attrs);
    }

    @Override
    public DragRecyclerView.LayoutParams generateLayoutParams(ViewGroup.LayoutParams lp) {
    ...
    @Override
    public boolean checkLayoutParams(DragRecyclerView.LayoutParams lp) {
    ...
    private void ensureAnchorIsInFirstSpan(DragRecyclerView.Recycler recycler, DragRecyclerView.State state,
                                           AnchorInfo anchorInfo) {
    ...
    private int getSpanGroupIndex(DragRecyclerView.Recycler recycler, DragRecyclerView.State state,
            int viewPosition) {
        if (!state.isPreLayout()) {
        ...
        if (adapterPosition == -1) {
            ...
            sLog.w(TAG, "Cannot find span size for pre layout position. " + viewPosition);
            return 0;
        }
        return mSpanSizeLookup.getSpanGroupIndex(adapterPosition, mSpanCount);
    }

    private int getSpanIndex(DragRecyclerView.Recycler recycler, DragRecyclerView.State state, int pos) {
        if (!state.isPreLayout()) {
        ...
        if (adapterPosition == -1) {
            ...
            sLog.w(TAG, "Cannot find span size for pre layout position. It is"
                    + " not cached, not in the adapter. Pos:" + pos);
            return 0;
        }
    ...
    private int getSpanSize(DragRecyclerView.Recycler recycler, DragRecyclerView.State state, int pos) {
        if (!state.isPreLayout()) {
    ...
    private void assignSpans(DragRecyclerView.Recycler recycler, DragRecyclerView.State state, int count,
     ...
     */
    public static class LayoutParams extends DragRecyclerView.LayoutParams {
        ...
        public LayoutParams(DragRecyclerView.LayoutParams source) {
            super(source);
        }
         ...
         * <p>
         * Note that span index may change by whether the DragRecyclerView is RTL or not. For
